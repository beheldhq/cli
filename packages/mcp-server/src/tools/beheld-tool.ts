...
function formatSummary(scores: EngineScores, insights: string[]): string {
  if (scores.sessions_analyzed === 0) {
    return "Beheld: no sessions analyzed yet. Keep using Claude Code — check back after a few sessions.";
  }
  // R1.2c — overall may be null when every dimension is absent.
  const overallTxt = scores.overall === null ? "—/100" : `${scores.overall}/100`;
  const lines: string[] = [
    `Overall score: ${overallTxt}  ${bar(scores.overall)}  (${scores.sessions_analyzed} sessions)`,
    "",
  ];
  ...
  if (insights.length === 0) {
    lines.push("→ Keep using Claude Code to generate personalized insights.");
  }
  return lines.join("\n");
}
...
function formatScores(scores: EngineScores): string {
  if (scores.sessions_analyzed === 0) {
    return "Beheld: no sessions analyzed yet.";
  }
  return [
...
function formatInsight(insights: string[]): string {
  ...
  if (!next) return "Beheld: no insights available right now.";
  return `→ ${next}`;
}

function formatFull(scores: EngineScores, summary: ProfileSummary | null, insights: string[]): string {
  if (scores.sessions_analyzed === 0) {
    return "Beheld: no sessions analyzed yet. Keep using Claude Code.";
  }
  ...
  if (summary && summary.total_sessions > 0) {
    ...
    if (summary.platforms.length > 0) {
      lines.push(`Platforms    ${summary.platforms.slice(0, 5).join(" · ")}`);
    }
...
export function formatImportResult(url: string, result: ImportResult): string {
  ...
  lines.push("");
  lines.push("Profile updated. L1 now includes this repository.");
  return lines.join("\n");
}
...
export async function handleImport(
  ...
  if (!health) {
    return "⚠ Engine offline. Run `beheld start` and try again.";
  }
  ...
  if (!authorEmail) {
    return [
      "⚠ Commit email not configured.",
      "",
      "Run in the terminal to configure:",
      "  beheld import",
      "",
      "The import wizard asks for the email once and saves it for future use.",
    ].join("\n");
  }
  ...
  while (Date.now() < deadline) {
    ...
    if (!r) {
      return `✗ Failed to import ${trimmed}\n  Empty response from engine.`;
    }
    ...
    switch (r.status) {
      ...
      case "already_imported":
        return `✓ ${trimmed}\n  Already imported. Reimporting does not change the profile.`;
      case "needs_pat":
        return [
          `⚠ Private repository: ${trimmed}`,
          "",
          "Private repositories require authentication.",
          "Use the terminal — the CLI asks for the token without exposing it in chat history:",
          "  beheld import",
        ].join("\n");
      case "author_not_found":
        return [
          `⚠ No commits found for ${authorEmail} in:`,
          `  ${trimmed}`,
          "",
          "Check that the email is correct in ~/.beheld/config.json",
        ].join("\n");
      case "clone_error":
        return `✗ Could not clone: ${trimmed}\n  Check the URL and try again.`;
      default: {
  ...
  return [
    `⏱ Import in progress: ${trimmed}`,
    "",
    "The repository continues processing in the background.",
    "Track progress with: beheld import --list",
  ].join("\n");
}
...
export function formatStackResponse(stack: StackResponse): string {
  ...
  if (strong.length === 0 && weak.length === 0) {
    lines.push("No architecture patterns identified.");
  } else {
    if (strong.length > 0) {
      const repoCount = strong.reduce((acc, p) => Math.max(acc, p.repo_count), 0);
      lines.push(`Patterns detected in ${repoCount} repositor${repoCount === 1 ? "y" : "ies"}:`);
      lines.push("");
      lines.push(strong.map((p) => patternLabel(p.pattern)).join(" · "));
    }
    if (weak.length > 0) {
      if (strong.length > 0) lines.push("");
      lines.push(`Hints: ${weak.map((p) => patternLabel(p.pattern)).join(" · ")}`);
    }
  }
...
export const beheldTool: McpTool = {
  ...
  async handler(args) {
    ...
    if (!scores) {
      return "Beheld: engine offline and no cached scores available. Run: beheld start";
    }

...
export async function installClaudeCodeHooks(
  ...
  for (const target of protectedTargets) {
    if (!target.startsWith(home)) {
      throw new Error(`Security: attempt to write outside home: ${target}`);
    }
    if (cwd !== home && target.startsWith(cwd + "/")) {
      throw new Error(`Security: attempt to write inside the current project: ${target}`);
    }
  }
 ...
 *   - greeting instruction with `[name]` placeholder (Claude resolves at runtime;
...
---
Before any response, introduce yourself with exactly this sentence,
...
──────────────────────────────────────────────────────────
ROUTING RULES (apply exactly, in this order)
──────────────────────────────────────────────────────────

Rule 1 — Conversational b3 mode:
  If "$ARGUMENTS" starts with "b3 " or "B3 " (case-insensitive, with a trailing space):
  → Extract the question after the prefix
  → If the question requires profile data (evolution, scores, patterns, commits,
    test ratio, ecosystems): call the \`beheld\` tool with action="view" and use
    the returned data to ground the response
  → If the question is contextual to code visible in the conversation:
  ...
  → Voice: third person. The bold **B3H31D** IS the subject of the first
    ...
    ("notices", "observes", "sees", "understands"). NEVER repeat the name
    "B3H31D" in the body of the response. Subsequent sentences chain
  ...
  → WRONG EXAMPLE (do NOT do this — subject repeated):

      **B3H31D** B3H31D notices... B3H31D observes... B3H31D understands...

  → TYPOGRAPHY — ABSOLUTE RULE: ZERO ITALICS in any part of the response.
    Explicit prohibitions:
      - NEVER use single asterisk *word* — that renders as italic
      - NEVER use underscore _word_ — also renders as italic
      - NEVER use blockquote (>) — the CLI renders blockquote in italic
      - NEVER use HTML tags <em>, <i>, or any equivalent markup
    Only allowed:
      - **word** (DOUBLE asterisk) for the B3H31D name in the template
      - plain text, no typographic emphasis
    If you want to highlight a term, put it in quotes: "like so".
    Never in italic. Upright typography is part of B3H31D's identity.

Rule 2 — Import with URL:
  If "$ARGUMENTS" starts with "import " (with a space, case-sensitive):
  → Extract everything after "import " as the URL
  → Call the \`beheld\` tool with: action="import", url=<extracted url>
  → Return exactly what the tool returns

Rule 3 — Import without URL:
  If "$ARGUMENTS" is exactly "import" (nothing after):
  → Call the \`beheld\` tool with: action="import", url=""
  → Return exactly what the tool returns

Rule 4 — Stack:
  If "$ARGUMENTS" is exactly "stack" OR "languages" OR "frameworks" OR "architecture":
  → Call the \`beheld\` tool with: action="stack"
  → Return exactly what the tool returns

Rule 5 — View (default):
  In any other case (empty, "summary", "scores", "insights", "full", etc.):
  → Call the \`beheld\` tool with: action="view", view="$ARGUMENTS" (default: "summary")
  → Return exactly what the tool returns
`;

...
export function formatRepoTable(repos: L1Repository[]): string {
  if (repos.length === 0) return `  ${DIM}No repositories imported.${RESET}`;
  const headerLine = `  ${DIM}HASH      IMPORT DATE       COMMITS${RESET}`;
...
async function ensureAuthorEmail(io: ImportIO, cfg: ImportConfigStore): Promise<string> {
  ...
  const email = (await io.prompt(`  What's your git commit email? ${meta("(e.g. eduardo@example.com)")}: `)).trim();
  if (!email) throw new Error("Git email is required to import.");
...
async function pollUntilTerminal(
  ...
  throw new Error("timed out waiting for repository ingestion.");
}
...
async function importOne(
  ...
  if (!accepted) {
    return { kind: "skipped", reason: "engine unavailable. check that the daemon is running." };
  }
  ...
  if (terminal.result?.status === "needs_pat") {
    io.log(warn("authentication required for this repository."));
    io.log(`     ${DIM}Create a token at github.com/settings/tokens (scope: repo — read-only).${RESET}`);
    ...
    pat = null;
    if (!reaccepted) {
      return { kind: "skipped", reason: "failed to resubmit with token." };
    }
    terminal = await pollUntilTerminal(io, client, pollIntervalMs);
  }

  const r = terminal.result;
  if (!r) return { kind: "skipped", reason: "empty response from engine." };

  switch (r.status) {
    case "imported":
      io.log(ok(`${bold(String(r.commit_count ?? 0))} commits imported — added to L1`));
      return { kind: "imported", commits: r.commit_count ?? 0 };
    case "already_imported":
      io.log(warn(`Already in L1 ${meta(`(hash ${shortHash(r.root_commit_hash ?? "")})`)} — skipped`));
      return { kind: "skipped", reason: "already_imported" };
    case "author_not_found":
      io.log(warn("no commits of yours found in this repository — skipped"));
      return { kind: "skipped", reason: "author_not_found" };
    case "clone_error":
      io.log(fail(`error accessing repository: ${r.detail ?? "unknown"}`));
      io.log(`     ${DIM}Check the URL and try again.${RESET}`);
      return { kind: "skipped", reason: "clone_error" };
    case "needs_pat":
      // Reached only if the second attempt still asks for a PAT.
      io.log(fail("authentication still required — skipped"));
...
export async function runImport(
  ...
  if (flags.list) {
    io.log(brand("repositories I've mapped"));
  ...
  if (flags.remove) {
    io.log(brand("removing a repository"));
    const hash = flags.remove;
    const confirmed = await io.confirm(
      `  ${bold("Remove repository")} ${shortHash(hash)} from L1? ${meta("(this cannot be undone)")} [y/N] `,
    );
    if (!confirmed) {
      io.log(warn("operation cancelled"));
      return;
    }
    const deleted = await client.deleteL1Repository(hash);
    io.log(deleted ? ok(`Repository ${shortHash(hash)} removed from L1`) : fail("repository not found"));
    return;
  }

  io.log(brand("bringing in your history"));
  ...
  if (flags.url) {
    ...
    io.log("");
    io.log(ok(`Bootstrap complete ${meta(`· ${count} repository(ies) · ${commits} commits analyzed`)}`));
    return;
  }
  ...
  let totalCommits = 0;
  while (true) {
    const url = (await io.prompt(`  Repository URL ${meta("(Enter to finish)")}: `)).trim();
    if (!url) break;
    const r = await importOne(url, authorEmail, io, client, pollIntervalMs);
    if (r.kind === "imported") {
      importedCount += 1;
  ...
  io.log("");
  io.log(ok(`Bootstrap complete ${meta(`· ${importedCount} repository(ies) · ${totalCommits} commits analyzed`)}`));
}

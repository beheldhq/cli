...
export async function runHostImport(
  ...
  deps.log(arrow(`Connecting to ${HOST_LABEL[host]}...`));
  ...
  deps.log(arrow("Fetching repositories..."));
  ...
  try {
    ...
    if (e instanceof HostListError) {
      ...
      deps.log(fail(`unexpected failure: ${(e as Error).message}`));
    }
    scrubToken(token);
    return summary;
  } finally {
  ...
  deps.log(ok(`${bold(String(repos.length))} repositories found.`));
  ...
  if (selected.length === 0) {
    deps.log(warn("No repositories selected."));
    return summary;
  }

  for (const repo of selected) {
    deps.log("");
    deps.log(arrow(`Importing ${bold(repo.full_name)}...`));
  ...
  deps.log(
    ok(
      `Bootstrap complete. ${meta(
        `${summary.imported} imported · ${summary.already_existing} already existed · ${summary.no_commits} with no commits of yours`,
      )}`,
    ),
  );
  return summary;
...
async function ingestWithSshFallback(
  ...
  if (hasSsh) {
    ...
    log(`     ${DIM}→ SSH unavailable, falling back to HTTPS.${RESET}`);

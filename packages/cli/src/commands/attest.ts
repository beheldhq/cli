...
export async function attestCommand(opts: AttestOptions = {}): Promise<void> {
  ...
  console.log(brand("verifying GitHub identity"));
  ...
  console.log(arrow(`starting local callback server`));
  ...
  console.log(arrow(`opening browser at ${meta(baseUrl)}`));
  await openBrowser(startUrl).catch(() => {
    console.log(arrow(`could not open browser automatically`));
    console.log(arrow(`open manually: ${startUrl}`));
  });
  ...
  try {
    ...
    session.stop();
    console.error(fail(`flow interrupted: ${(err as Error).message}`));
    process.exit(1);
  }

  console.log(arrow(`receiving attestation`));
  const attestation = await claimAttestation(baseUrl, claimCode);

  saveAttestationCache(attestation, opts.dataDir);

  console.log(ok("identity attested"));
...
<html><head><title>Beheld — authorized</title>
<meta charset="utf-8">
<style>
...
function startCallbackServer(opts: { cliState: string; timeoutMs: number }): CallbackSession {
  ...
  const timeoutHandle = setTimeout(() => {
    rejectClaim(new Error("timed out waiting for OAuth callback (5 min)"));
    ...
    fetch(req) {
      ...
      try {
        ...
        if (cliState !== opts.cliState) {
          rejectClaim(new Error("cli_state mismatch — possible CSRF"));

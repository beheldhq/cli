...
/** Single-letter affirmative ("y", case-insensitive). Empty input
...
export async function runShare(deps: ShareDeps = {}): Promise<ShareOutcome> {
  ...
  if (bundlePath === null) {
    out(fail("no bundle found. Run: beheld snapshot"));
  ...
  try {
    ...
    out(fail(`failed to read local bundle: ${(e as Error).message}`));
    return { ok: false, exitCode: 1 };
  }
  ...
  if (shouldPromptEmail) {
    ...
    try {
      out("→ Register an email for account recovery? [y/N]");
      out("  (recommended — needed to regain access if you lose this device)");
  ...
  if (!result.ok) {
    out(fail("upload failed — bundle saved locally"));
    if (result.error.kind === "network") {
      out(`  ${DIM}network: ${result.error.message}${RESET}`);
    } else {
      out(`  ${DIM}HTTP ${result.error.status}:${RESET} ${result.error.body.slice(0, 200)}`);
    }
    out(`  ${DIM}Try again: beheld share${RESET}`);
...
export async function shareCommand(): Promise<void> {
  console.log(brand("publishing profile"));

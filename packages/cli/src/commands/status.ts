...
export async function statusCommand(): Promise<void> {
  ...
  if (session?.active) {
    ...
    console.log(`  ${BOLD}Current session${RESET}  ${dur} min · ${evts} events${tools ? ` · ${tools}` : ""}`);
  } else {
    console.log(`  ${BOLD}Current session${RESET}  ${DIM}no active session${RESET}`);
  }
  ...
  if (status) {
    ...
    console.log(
      `  ${BOLD}Today${RESET}           ${sessionsToday} sessions · ${eventsToday} events`,
    );
  }

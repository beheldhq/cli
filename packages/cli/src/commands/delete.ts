...
async function askConfirmPhrase(phrase: string): Promise<boolean> {
  ...
  return new Promise((resolve) => {
    rl.question(`  Type "${phrase}" to confirm: `, (ans) => {
      rl.close();
...
function removeMacosDevprofileResidue(home: string): ResidueResult {
  ...
  try {
    unlinkSync(plist);
    return { found: true, detail: "LaunchAgent removed" };
  } catch (err) {
    return { found: true, detail: `failed to remove plist: ${(err as Error).message}` };
  }
}
...
function removeLinuxDevprofileResidue(home: string): ResidueResult {
  ...
  try {
    unlinkSync(service);
    return { found: true, detail: "systemd unit removed" };
  } catch (err) {
    return { found: true, detail: `failed to remove unit: ${(err as Error).message}` };
  }
}
...
function scrubClaudeSettingsDevprofile(home: string): ResidueResult {
  ...
  try {
    ...
    return { found: true, detail: "devprofile entries removed" };
  } catch (err) {
    return { found: true, detail: `failed: ${(err as Error).message}` };
  }
}
...
async function runRemote(): Promise<void> {
  console.log(
    `\n  ${RED}This will invalidate already-shared bundles that reference this attestation.${RESET}`,
  );
  const confirmed = await askConfirmPhrase("revoke");
  if (!confirmed) {
    console.log("Aborted.");
    return;
  }

  startStep("Revoking attestation on server");
  const result = await revokeRemoteAttestation();
  switch (result) {
    case "revoked":
      okStep("Attestation revoked on server");
      break;
    case "not_attested":
      skipStep("No local attestation found — nothing to revoke");
      break;
    case "server_offline":
      failStep("Server unavailable", new Error("could not connect"));
      console.log(
        `  ${DIM}Try again when the server is reachable.${RESET}`,
      );
      break;
    case "failed":
      failStep("Server rejected the revocation", new Error("422 or similar"));
      break;
  }
}
...
async function runLocal(opts: { skipConfirm?: boolean } = {}): Promise<void> {
  ...
  if (!opts.skipConfirm) {
    ...
    console.log(
      `\n  ${RED}This will delete ${total} sessions of local data. Cannot be undone.${RESET}`,
    );
    const confirmed = await askConfirmPhrase("delete everything");
    if (!confirmed) {
      console.log("Aborted.");
      return;
    }
  ...
  startStep("Stopping daemon");
  try {
    await daemonManager.stop();
    okStep("Daemon stopped");
  } catch {
    skipStep("Daemon was not running");
  }

  startStep("Deleting ~/.beheld/");
  try {
    if (existsSync(dir)) rmSync(dir, { recursive: true, force: true });
    okStep("~/.beheld/ removed");
  } catch (err) {
    failStep("~/.beheld/ removed", err);
  }
}
...
async function runAll(): Promise<void> {
  ...
  console.log(
    `\n  ${RED}Full removal: ${total} local sessions, server attestation, hooks, and old-name residue.${RESET}`,
  );
  const confirmed = await askConfirmPhrase("delete everything");
  if (!confirmed) {
    console.log("Aborted.");
  ...
  if (hasAttestation) {
    startStep("Revoking attestation on server");
    const result = await revokeRemoteAttestation();
    if (result === "revoked") okStep("Attestation revoked on server");
    else if (result === "server_offline") {
      process.stdout.write(
        `\r  ${DIM}⚠${RESET}  Server unavailable — attestation not revoked remotely.\n`,
      );
      console.log(
        `      ${DIM}To revoke later: beheld delete --remote${RESET}`,
      );
    } else if (result === "failed") {
      process.stdout.write(
        `\r  ${DIM}⚠${RESET}  Server rejected the revocation — continuing with local cleanup.\n`,
      );
    } else {
      skipStep("No local attestation to revoke");
    }
  } else {
    skipStep("No local attestation to revoke");
  }
  ...
  startStep("Removing ~/.beheld/");
  try {
    if (existsSync(dir)) rmSync(dir, { recursive: true, force: true });
    okStep(`~/.beheld/ removed (${total} sessions)`);
  } catch (err) {
    failStep("~/.beheld/ removed", err);
  }

  // 4) Remove integration hooks (Claude Code + Continue.dev + slash command + claude mcp)
  startStep("Removing hooks/MCP");
  try {
    await removeAllHooks();
    okStep("Hooks/MCP removed");
  } catch (err) {
    failStep("Hooks/MCP removed", err);
  }

  // 5) Cleanup devprofile rename residues
  startStep("Cleaning up old-name residue (devprofile)");
  const residue = cleanupDevprofileResidues();
  if (residue.foundAny) {
    process.stdout.write(`\r  ${GREEN}✓${RESET}  devprofile residue cleaned:\n`);
    for (const m of residue.messages) {
      console.log(`      ${DIM}${m}${RESET}`);
    }
  } else {
    skipStep("No devprofile residue found");
  }

  // Footer
  console.log(`\n  ${BOLD}Beheld removed successfully.${RESET}`);
  console.log(`\n  To remove the binary:`);
  console.log(`    rm $(which beheld)`);
  console.log(`\n  Verify cleanup:`);
  console.log(`    which beheld && echo "binary still present" || echo "✓ binary removed"`);
...
export async function deleteCommand(opts: DeleteOptions): Promise<void> {
  console.log(brand("deleting what's left"));
  const { local, remote, all } = opts;

  if (!local && !remote && !all) {
    console.error("  Specify --local, --remote, or --all");
    process.exit(1);
  }

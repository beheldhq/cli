...
async function fetchPage(
  ...
  try {
    ...
    if (err instanceof Error && err.name === "AbortError") {
      throw new HostListError("timed out while listing repositories.", host);
    }
    throw new HostListError(
      `network error contacting ${host}: ${(err as Error).message}`,
      host,
    );
  } finally {
    clearTimeout(timer);
  ...
  switch (res.status) {
    case 401:
      throw new HostListError(
        "invalid credentials. check the token and try again.",
        host,
        401,
      );
    case 403:
      throw new HostListError(
        "insufficient permissions. check the token scopes.",
        host,
        403,
      );
    case 404:
      throw new HostListError("user not found.", host, 404);
    case 429:
      throw new HostListError(
    ...
    default:
      if (res.status >= 500) {
        throw new HostListError(
          `${host} server error. try again later.`,
          host,
          res.status,
        );
      }
      throw new HostListError(
        `unexpected response from ${host} (HTTP ${res.status}).`,
        host,
        res.status,
      );
  }
...
export async function fetchHostRepos(
  ...
  if (host === "bitbucket") {
    if (cred.method !== "app_password") {
      throw new HostListError(
        "Bitbucket requires username + app password.",
        "bitbucket",
      );
    }
  ...
  if (cred.method === "app_password") {
    throw new HostListError(
      `${host} does not use app password.`,
      host,
    );
  }
  return host === "github"

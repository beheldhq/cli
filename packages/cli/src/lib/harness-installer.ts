...
export interface HarnessAdapter {
   ...
   *  specific particulars (detected path, hook mechanism, etc.) —
   *  complements the generic explanation derived from `fidelity`. Written
   *  in the product's witness voice. */
...
const claudeAdapter = (): HarnessAdapter =>
  makeBuiltinHooksAdapter({
    ...
    description: "PreToolUse/PostToolUse/Stop in ~/.claude/settings.json + /beheld slash command + global MCP",
...
const continueAdapter = (): HarnessAdapter =>
  makeBuiltinHooksAdapter({
    ...
    description: "MCP server at http://localhost:7337/mcp registered in ~/.continue/config.json",
...
const windsurfAdapter = (): HarnessAdapter => ({
  ...
  fidelity: "native_hook",
  description: "Cascade Hooks at ~/.codeium/windsurf/hooks.json (auto backup before writing)",
...
const geminiAdapter = (): HarnessAdapter =>
  makeManualAdapter({
    ...
    label: "Gemini CLI",
    fidelity: "native_hook",
    description: "manual — Gemini's hook spec not yet published; handler ready at /hook/gemini/*",
...
const codexAdapter = (): HarnessAdapter =>
  makeManualAdapter({
    ...
    label: "Codex CLI",
    fidelity: "native_hook",
    description: "manual — Codex's hook spec not yet published; handler ready at /hook/codex/*",
...
const cursorAdapter = (): HarnessAdapter =>
  makeTailAdapter({
    ...
    description: "tails ~/Library/Application Support/Cursor (macOS) or ~/.config/Cursor (Linux)",
    detectionPaths: [
...
const copilotCliAdapter = (): HarnessAdapter =>
  makeTailAdapter({
    ...
    label: "Copilot CLI",
    fidelity: "statusline",
    description: "polls statusline in ~/.local/share/gh-copilot, ~/.cache/gh-copilot or Library/Application Support/gh-copilot",
    detectionPaths: [
...
const copilotVscodeAdapter = (): HarnessAdapter =>
  makeTailAdapter({
    ...
    fidelity: "local_log_tail",
    description: "tails ~/Library/Application Support/Code/logs (macOS) or ~/.config/Code/logs (Linux)",
    detectionPaths: [

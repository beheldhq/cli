...
const DISCIPLINE_LABEL: Record<string, string> = {
  ...
  moderate: "Moderate",
  low: "Forming",
  minimal: "Little evidence",
};

const APPROACH_LABEL: Record<string, string> = {
  tdd_dominant: "TDD in most sessions",
  tdd_partial: "TDD in a sizable share of sessions",
  test_after: "Tests written after the code",
  test_seldom: "Tests rarely written",
  exploratory: "Exploratory sessions",
};
...
const WORKFLOW_LABEL: Record<string, string> = {
  ...
  refactor_heavy: "Refactor before review",
...
const PEAK_LABEL: Record<string, string> = {
  ...
  afternoon: "Concentrated in the afternoon",
...
function formatEmergent(e: EmergentDiff): string {
  ...
  if (olderPct < 5) {
    return `${label} appears in ${recentPct}% of sessions over the last ${e.recent_window_days} days. ` +
           `Before that it was the exception; now it's the norm.`;
  }
  return `${label} grew from ${olderPct}% to ${recentPct}% of sessions in the last ${e.recent_window_days} days.`;
}
...
const STACK_PATTERN_LABELS: Record<string, string> = {
  ...
  iac: "IaC",
  container_orchestration: "Orchestration",
  ci_cd: "CI/CD",
};
...
function renderStackSection(stack: BundleStackSection | null | undefined): string {
  ...
  if (patterns.length === 0) {
    archHtml = `<p class="placeholder">No patterns identified</p>`;
  } else {
...
function renderPerfilTecnicoSection(enrichment: EnrichmentRendererView | undefined): string {
  ...
  return `
    <section class="perfil-tecnico" aria-label="Perfil técnico">
      ...
      <div class="perfil-row"><span class="key">Platforms</span><span class="val">${escapeHtml(hasPlat ? platLabel : "—")}</span></div>
...
const TIER_BADGE: Record<TrustTier, TierBadgeSpec> = {
  fully_verifiable: {
    label: "Verified · Sigstore Rekor",
    variant: "trusted",
    hint: "Signed bundle + GitHub identity + public inclusion in the Sigstore Rekor log.",
  },
  engine_verified: {
    label: "GitHub identity + engine verified",
    variant: "strong",
    hint: "Signed bundle + GitHub identity + hash of the engine that produced the profile.",
  },
  identity_verified: {
    label: "GitHub identity verified",
    variant: "good",
    hint: "Signed bundle + GitHub identity bound to the developer's key.",
  },
  chain_intact: {
    label: "Signed · chain intact",
    variant: "neutral",
    hint: "Signed bundle + linked to the prior snapshot chain.",
  },
  signature_only: {
    label: "Locally signed",
    variant: "neutral",
    hint: "Signed with the developer's key. No additional external proof.",
  },
  unsigned: {
    label: "Unsigned",
    variant: "neutral",
    hint: "Bundle has no signature — authorship cannot be verified.",
  },
};
...
function formatIsoToHuman(iso: string | undefined): string {
  ...
  if (!gh || !hasSig) {
    return `
        <div class="trust-section trust-section-muted">
          <p class="trust-section-title">GitHub identity</p>
          <p class="trust-section-body">Not linked. <code>beheld attest</code> binds the developer's key to a public GitHub identity.</p>
        </div>`;
  }
  ...
  return `
        <div class="trust-section">
          ...
          <p class="trust-section-body">
            ...
            <span class="trust-meta"> · user id ${gh.user_id} · verified on ${escapeHtml(formatIsoToHuman(gh.verified_at))}</span>
          </p>
        </div>`;
}
...
function renderRekorSection(rekor: BundleRekorView | null | undefined): string {
  ...
  if (!hasRekor) {
    return `
        <div class="trust-section trust-section-muted">
          ...
          <p class="trust-section-body">Not submitted (network unavailable at generation time).
          Re-submit: <code>beheld snapshot --rekor-submit &lt;bundle&gt;</code></p>
        </div>`;
  }
  ...
  const integratedLabel = formatIsoToHuman(rekor!.integratedTime);
  return `
        <div class="trust-section">
          <p class="trust-section-title">🎉 Sigstore Rekor</p>
          <p class="trust-section-body">
...
export function renderTrustDetails(bundle: Bundle): string {
  return `
        <p style="margin-bottom: 12px;">
          ...
          Verification happens in this browser, with no server call.
        </p>
...
export function renderSnapshotHtml(data: SnapshotHtmlData): string {
  ...
  const emergentBlock = data.emergent
    ...
    <section class="emergent">
      <div class="label">Emerging pattern</div>
  ...
  const captureLine = repoCount > 0
    ? `Captured from ${repoCount} repositor${repoCount === 1 ? "y" : "ies"} and months of real usage — not self-declared.`
    : "Captured from real Claude Code usage — not self-declared.";
    ...
    <footer class="footer">
      <div class="verification" data-status="checking" id="verification">
        ...
        <button class="verification-toggle" id="verification-toggle" aria-expanded="false" aria-controls="verification-details">
          <span id="verification-label">Verifying signature…</span>
        </button>
      </div>
